...
from loss_vkitti_single_only import MultiTaskLossVkitti
from loss_nyuv2_single_only import MultiTaskLossNYU
from loss_cityscapes_single_only import MultiTaskLossCity
from architecture.unet.unet_model_single_only import UNet
...
def train(epoch):
    ...
    for iteration, batch in enumerate(train_loader):
        ...
        loss = criterion(masks_pred, deps_pred, true_masks, true_deps)
...
def accuracy_vkitti(masks_pred, deps_pred, true_masks, true_deps, reverse):
    ...
    if reverse:
        ...
        for tr_mask, pred_mask in zip(true_masks, masks_pred):
            seg_loss += F.cross_entropy(pred_mask.unsqueeze(dim=0), tr_mask.unsqueeze(dim=0)).item()  # add segmentation loss
    else:
        for tr_dep, pred_dep in zip(true_deps, deps_pred):
            dep_l1_loss += F.l1_loss(pred_dep, tr_dep).item()
            dep_rmse += torch.sqrt(F.mse_loss(pred_dep, tr_dep)).item()

    return seg_loss, seg_iou, dep_l1_loss, dep_rmse


def accuracy_nyuv2(masks_pred, deps_pred, true_masks, true_deps, reverse):
    import torch.nn
    import torch.nn.functional as F
    seg_loss = 0
    dep_l1_loss = 0
...
def test():
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            if args.dataset == 'vkitti':
                acc_ret = accuracy_vkitti(masks_pred, deps_pred, true_masks, true_deps, args.reverse)
                tot_seg += acc_ret[0]
                tot_iou += acc_ret[1]
                tot_dep_l1 += acc_ret[2]
                tot_dep_rmse += acc_ret[3]
            elif args.dataset == 'nyuv2' or args.dataset == 'cityscapes':
                acc_ret = accuracy_nyuv2(masks_pred, deps_pred, true_masks, true_deps, args.reverse)
                tot_seg += acc_ret[0]
                tot_iou += acc_ret[1]
                tot_dep_l1 += acc_ret[2]
                tot_dep_rmse += acc_ret[3]
    ...
    if args.reverse:
        ...
        print("===> Avg. Seg IOU: {:.3f} (%)\n".format(tot_iou * 100 / len(test_loader)))
    else:
        print("===> Avg. L1Loss inverse depth: {:.6f} (m^-1)".format(tot_dep_l1 / len(test_loader)))
        print("===> Avg. RMSE inverse depth: {:.5f} (m^-1)\n".format(tot_dep_rmse / len(test_loader)))
    return
...
if __name__ == '__main__':
    ...
    if args.model == 'unet':
        ...
        model = UNet(n_channels=3, n_classes=configs['n_classes'], reverse=args.reverse).cuda(args.gpu)
    ...
    if args.dataset == 'vkitti':
        criterion = MultiTaskLossVkitti(device, args.reverse)
    elif args.dataset == 'nyuv2':
        criterion = MultiTaskLossNYU(device, args.reverse)
    elif args.dataset == 'cityscapes':
        criterion = MultiTaskLossCity(device, args.reverse)
    # criterion = MultiTaskLoss(device, args.reverse)
