...
class MultiTaskLossVkitti(nn.Module):
    ...
    def __init__(self, n_classes, device, reverse=False):
        ...
        if reverse:
            self.n_classes = len(MultiTaskLossVkitti.depth_class_range) + 1 # +1 including background
        else:
        ...
        moment_vector1 = torch.zeros(self.n_classes)
        moment_vector2 = torch.zeros(self.n_classes)
    ...
    def _get_depth_onehot_mask(self, true_deps, deps_shape):
        ...
        for dd in MultiTaskLossVkitti.depth_class_range:
            t = true_deps < 1 / dd
            deps_mask_idx += t.cpu().numpy()
        deps_onehot_mask = torch.FloatTensor(torch.Size(
            [deps_shape[0], self.n_classes, deps_shape[2],
             deps_shape[3]]))  # new tensor of shape (B, C, H, W)
        deps_onehot_mask = deps_onehot_mask.to(device=self.device, dtype=torch.float32)
        deps_onehot_mask.zero_()  # set all elements as 0
    ...
    def forward(self, loss_weight, masks_pred, deps_pred, true_masks, true_deps):
        ...
        self.diff_aux = self.diff_aux / self.pixel_num
        ...
        for i in range(self.n_classes):
            ...
            loss_aux.append(t)
        loss_aux = torch.stack(loss_aux, 0) / self.pixel_num
